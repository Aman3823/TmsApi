...
public class AuthController : ControllerBase
{
    ...
    public AuthController(
        ...
        RoleManager<IdentityRole> roleManager, // <--- ኮማዋ ተጨምራለች
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request)
    {
        ...
        // 1. አካውንቱ ተቆልፎ እንደሆነ ማረጋገጥ (Lockout Check)
        if (await _userManager.IsLockedOutAsync(user))
        {
            return StatusCode(423, new { detail = "Account locked due to multiple failed login attempts. Try again in 15 minutes." });
        }

        // 2. የይለፍ ቃሉን ማረጋገጥ(password check)
        var validpassword = await _userManager.CheckPasswordAsync(user, request.Password);
        if (!validpassword)
        {
            // የተሳሳተ ሙከራን መዝግቦ ቁጥሩን መጨመር
            await _userManager.AccessFailedAsync(user);
            return Unauthorized(new { detail = "Invalid credentials." });
        }

        // 3. በስኬት ሲገባ የተሳሳቱ ሙከራዎች ቁጥርን ወደ zero መመለስ
        await _userManager.ResetAccessFailedCountAsync(user);

        // 4. JWT Access Token እና Refresh Token ማመንጨት
    ...
    public async Task<IActionResult> Refresh([FromBody] RefreshRequest request)
    {
        ...
        // Theft Detection: ያገለገለ (Used) ቶከን ከተላከ የተጠቃሚውን ቶከኖች በሙሉ Revoke ማድረግ
        ...
        // አሁን የተላከውን ቶከን እንደ ጥቅም ላይ የዋለ (IsUsed) መመዝገብ
        storedToken.IsUsed = true;

        // አዲስ የ Refresh Token ጥንድ ማመንጨት
